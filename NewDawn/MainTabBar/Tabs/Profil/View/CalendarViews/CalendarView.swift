...
/**
 NewDawn
 Created by: Mathieu Janneau on 22/04/2018
...
struct Colors {
  ...
  static var  ligthGreen = UIColor.red
}
...
class CalenderView: UIView, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, MonthViewDelegate {
  ...
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    ...
    if indexPath.item <= firstWeekDayOfMonth - 2 {
      ...
      if calcDate < todaysDate && currentYear == presentYear && currentMonthIndex == presentMonthIndex {
       ...
       let formattedDate = "\(currentYear)-0\(currentMonthIndex)-\(calcDate)"
      if cellHasChallenge(dateToCheck: formattedDate) {
  ...
  func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    ...
    cell?.backgroundColor = UIConfig.darkGreen
    ...
    if cellHasChallenge(dateToCheck: formattedDate) {
      ...
      NotificationCenter.default.post(Notification(name:Notification.Name(rawValue: "calendarActive"), object: nil, userInfo: ["Key":"key", "isActive" : false]))
...
class dateCVCell: UICollectionViewCell {
  ...
  let selectedView: UIView = {
    ...
    circleView.backgroundColor = UIConfig.darkGreen
    return circleView
  }()
