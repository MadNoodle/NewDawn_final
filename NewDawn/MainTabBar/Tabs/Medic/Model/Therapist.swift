...
struct Therapist {
  ...
  static func getTherapist() -> [Therapist]{
    ...
    do {
      ...
      let dataSet = try JSONSerialization.jsonObject(with: jsonData) as! [[String:Any]]
              ...
              if let adresse = data["adresse"] as? String {
                if let tel = data["telephone"] as? String {
                  if let profession = data["profession"] as? String {
                    ...
                    let medic = Therapist(name: name, lat: lat, lng: lng, adresse: adresse, tel: tel, profession: profession)
